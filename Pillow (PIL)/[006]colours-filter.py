...
def draw_gradient(im, *colours, direction="diagonal"):
    ...
    for i in range(len(colours) - 1):
        for colour in _interpolate(colours[i], colours[i + 1]):
            if direction == "horizontal":
                draw.line(
                    [(line_number, 0), (line_number, im.height)], tuple(colour), width=1)
            elif direction == "vertical":
                draw.line(
                    [(line_number, 0), (im.width, line_number)], tuple(colour), width=1)
            else:
                draw.line([(line_number, 0), (0, line_number)],
                          tuple(colour), width=1)
...
def greyscale(im):
    return im.convert("L")  # Luminance: Single value between 0 to 255
...
if __name__ == "__main__":
    with Image.open("img/stargirl.jpg") as im:
        im = im.convert("RGBA")
        greyscale_im = greyscale(im)
        greyscale_im.save('saved/stargirl-grey.png')

        pride_im = pride(im, direction="horizontal")
        pride_im.save("saved/stargirl-pride.png")

        trans_im = trans_pride(im, direction="vertical")
        trans_im.save("saved/stargirl-trans.png")
